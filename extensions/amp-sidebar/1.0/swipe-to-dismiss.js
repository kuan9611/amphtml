...
export class SwipeToDismiss {
  ...
  getSwipeElementLength_() {
    return this.orientation_ == Orientation.HORIZONTAL
      ? this.swipeElements_[0]./*OK*/ offsetWidth
      : this.swipeElements_[0]./*OK*/ offsetHeight;
  }
   ...
   * }} config
   */
  startSwipe({swipeElements, direction, orientation}) {
    this.swipeElements_ = swipeElements;
  ...
  carrySwipeMomentum_(distance, velocity) {
    ...
    this.swipeElements_.forEach(element => {
    ...
    return this.mutateElement_(() => {
      this.swipeElements_.forEach(element => {
        setImportantStyles(element, {
          '--swipe-fraction': 0,
          'transition': `${duration}ms transform ease-in, ${duration}ms opacity ease-in`,
        });
      });
    }).then(() => {
      return delayAfterDeferringToEventLoop(this.win_, duration);
  ...
  dismissFromSwipe_(finalDistance) {
    ...
    return this.mutateElement_(() => {
      this.swipeElements_.forEach(element => {
        setImportantStyles(element, {
          '--swipe-fraction': 1,
          'transition': `${duration}ms transform ease-out, ${duration}ms opacity ease-out`,
        });
      });
    })
   ...
   * @private
   */
  adjustForSwipePosition_(swipeFraction = 0) {
    this.swipeElements_.forEach(element => {
      setImportantStyles(element, {
        '--swipe-fraction': swipeFraction,
        'transition': '',
      });
    });
  }
  ...
  swipeMove_(data, isLast) {
    ...
    this.mutateElement_(() => {
      ...
      const swipeFraction = clamp(
        swipeDistance / this.getSwipeElementLength_(),
        0,
        1
      );

      this.adjustForSwipePosition_(swipeFraction);
    });
  }
}
